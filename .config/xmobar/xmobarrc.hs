Config { 
    ...
    , commands =
	    ...
	    , "--normal" , "#de935f" 
	    , "--high" , "#a54242" 
	    , "--ppad" , "3" , "--width" , "3" , "--maxtwidth" , "4" , "--" , "--used-icon-pattern" , "<icon=ram/ram_%%.xpm/>"
            ] 10
        -- , Run Brightness
        --     [ "-t" , "<ipat>"
        --     , "--"
	...
	, Run Com "uname" ["-s", "-r"] "" 3600
        , Run DynNetwork 
	    [ "--template" , "<fc=#d5c4a1><dev></fc> <tx>kB/s|<rx>kB/s <txipat><rxipat>" 
	    , "--Low" , "10000" -- units: B/s 
	    ...
	    , "--high" , "#a54242" 
	    , "--maxtwidth" , "0" 
	    , "--" , "--rx-icon-pattern" , "<icon=network/rx/network_rx_%%.xpm/>" , "--tx-icon-pattern" , "<icon=network/tx/network_tx_%%.xpm/>"
            ] 10
	    ...
	    , "-i" , "<fc=#707880>Full</fc>" -- charged status
            ] 50
	, Run Kbd
            [ ("ar", "<fc=#cc6666>AR</fc>")
            , ("us", "<fc=#b5bd68>US</fc>")
	    ...
	    , ("/home", "<fn=1>\xf0a0</fn> <free>")
	    	...
	    	"--Low" , "10" , "--High" , "50" , "--low" , "red" , "--high" , "darkgreen" , "-m" , "1" , "-p" , "3"
            ] 100
        , Run MPD 
	    [ "--template" , "<artist> <title> <plength> <statei> " 
	    , "--maxtwidth" , "30" 
	    , "--" , "--host" , "127.0.0.1" 
